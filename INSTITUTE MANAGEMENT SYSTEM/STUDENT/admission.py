...
def logic():
    try:
        ...
        cr = course_entry.get()
        bt = batch_time_entry.get()
        stf = staff_entry.get()
        fs = fess_entry.get()
        dr = duration_entry.get()

        sql = "insert into adf2 values('" + dt + "','" + fn + "','" + ad + "','" + ct + "','" + mn + "','" + eid + "','" + cr + "','" + bt + "','" + stf + "','" + fs + "','" + dr + "')"
...
course_entry=ctk.CTkEntry(master=frame,placeholder_text="Course Name:")
course_entry.pack(pady=12,padx=10)

batch_time_entry=ctk.CTkEntry(master=frame,placeholder_text="Batch Time:")
batch_time_entry.pack(pady=12,padx=10)

staff_entry=ctk.CTkEntry(master=frame,placeholder_text="Staff Name:")
staff_entry.pack(pady=12,padx=10)

fess_entry=ctk.CTkEntry(master=frame,placeholder_text="Total Fess:")
fess_entry.pack(pady=12,padx=10)

duration_entry=ctk.CTkEntry(master=frame,placeholder_text="Total Duration:")
