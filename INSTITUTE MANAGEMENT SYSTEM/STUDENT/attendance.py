...
def logic():
    try:
        ...
        at = attendance_entry.get()
        nt = note_entry.get()
        stf = staff_entry.get()

        sql = "insert into atf values('" + dt + "','" + fn + "','" + at + "','" + nt + "','" + stf + "')"
...
attendance_entry=ctk.CTkEntry(master=frame,placeholder_text="Attendance [in %]:")
attendance_entry.pack(pady=12,padx=10)

note_entry=ctk.CTkEntry(master=frame,placeholder_text="Remarks:")
note_entry.pack(pady=12,padx=10)
